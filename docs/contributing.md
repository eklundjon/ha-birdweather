...
`custom_components/birdweather/manifest.json` is the single source of truth for
the version. The tag is derived from it, never the other way round, so the
version string is written in exactly one place.
